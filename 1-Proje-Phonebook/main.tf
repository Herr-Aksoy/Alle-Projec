...
resource "aws_launch_template" "asg-lt" {
  ...
  user_data              = filebase64("${path.module}/user-data.sh")  # base64encode(templatefile("${path.module}/user-data.sh", {})) # bu olmuyor base64encode("./user-data.sh")
  tag_specifications {
